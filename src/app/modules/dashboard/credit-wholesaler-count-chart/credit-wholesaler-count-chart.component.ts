...
@Component({
    selector: 'app-credit-wholesaler-count-chart',
    templateUrl: './credit-wholesaler-count-chart.component.html',
    styleUrls: ['./credit-wholesaler-count-chart.component.css']
})
export class CreditWholesalerCountChartComponent {
    ...
    private updateChart(selectedPeriod: number, wholesalerCode: string) {
        this.data$ = this.dashboardService.getCreditWholesalerCountChartData(selectedPeriod, wholesalerCode).pipe(share())
        this.data$.subscribe({
            next: (response) => {
    ...
    private createChart(data: any) {
        ...
        creditTypes.forEach(element => {
            switch (element) {
                case CreditRequestType.Overdraft :
                    datasetsValues.push({
                        label: `Wholesaler Credit Count last ${this.selectedPeriod} Days.`,
                ...
                case CreditRequestType.NanoCredit :
                    datasetsValues.push({
                        label: `Wholesaler Nano Credit Count last ${this.selectedPeriod} Days.`,
                        data: data.filter((data: any) => data.type == element),
                        fill: true,
        ...
        this.chart = new Chart("creditWholesalerCountChart", {
            type: 'line',
            data: {
